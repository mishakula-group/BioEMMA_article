...
MODELS = (
    ModelSpec("gapseq", "gapseq", "gapseq.sbml.xml"),
    ModelSpec("modelseed", "ModelSEED", "modelseed.sbml.xml"),
    ModelSpec("reconstructor", "Reconstructor", "reconstructor.sbml.xml"),
)
...
def rename_map(
    ...
    *,
    ...
    escher_map[0]["map_name"] = f"figure_03_{output_slug}_{model.slug}_{mode}"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build Figure 3 full KEGG maps for reconstruction-tool models."
    )
    parser.add_argument(
        "--pathway",
...
def main() -> None:
    ...
    if missing:
        raise FileNotFoundError("Missing Figure 3 model inputs: " + ", ".join(missing))
    ...
    write_json(output_root / "figure_03_summary.json", overview)

    for item in summaries:
        print(f"{item['tool']}: {item['map_html']}")
    print(f"summary: {output_root / 'figure_03_summary.json'}")
