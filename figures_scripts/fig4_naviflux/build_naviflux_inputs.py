...
# Figure 3 lower-glycolysis reactions retained on the e_coli_core/BioEMMA panel.
...
# Same BioEMMA layout knobs as Figure 3, to keep the comparison stable.
...
def write_naviflux_weight_files(fluxes: dict[str, float]) -> dict[str, Path]:
    ...
    paths = {
        "reaction_weights": OUTPUT_DIR / "naviflux_reaction_weights_map00010_reactions.csv",
        "flux_weights": OUTPUT_DIR / "naviflux_flux_weights_map00010_reactions.csv",
    }
...
def load_or_build_bioemma_map() -> list[dict[str, Any]]:
    if MAP00010_FULL_MAP.exists():
        with MAP00010_FULL_MAP.open("r", encoding="utf-8") as file:
...
def write_reduced_model() -> tuple[Path, Path, dict[str, Any]]:
    ...
    sbml_path = OUTPUT_DIR / "e_coli_core_map00010_reactions.xml"
    json_path = OUTPUT_DIR / "e_coli_core_map00010_reactions.json"
...
def main() -> None:
    ...
    official_filtered, official_meta = filter_escher_map(
        official_source,
        map_name="fig4_official_escher_ecoli_core_map00010_reactions",
        map_description=(
            "Official Escher e_coli_core map filtered to the Figure 3 "
            "lower-glycolysis reactions."
        ),
    )
    official_json = OUTPUT_DIR / "official_escher_ecoli_core_map00010_reactions.json"
    official_html = OUTPUT_DIR / "official_escher_ecoli_core_map00010_reactions.html"
    bioemma_json = OUTPUT_DIR / "bioemma_ecoli_core_map00010_reactions.json"
    bioemma_html = OUTPUT_DIR / "bioemma_ecoli_core_map00010_reactions.html"

    write_json(official_json, official_filtered)
    bioemma_filtered, bioemma_meta = filter_escher_map(
        bioemma_source,
        map_name="fig4_bioemma_ecoli_core_map00010_reactions",
        map_description=(
            "BioEMMA e_coli_core rn00010 map filtered to the Figure 3 "
            "lower-glycolysis reactions."
        ),
    )
